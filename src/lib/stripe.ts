...
function resolveStripeSecret() {
  ...
  if (process.env.NODE_ENV === "test" || process.env.CI === "true" || process.env.NEXT_PHASE === "phase-production-build") {
    // Vitest environment or CI build: use a deterministic mock key so the SDK can be constructed
    return "sk_test_mock";
  }
