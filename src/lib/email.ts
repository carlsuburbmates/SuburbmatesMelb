...
import { escapeHtml, stripNewlines } from './sanitization';
...
export async function sendTierDowngradeEmail(params: {
  ...
  const safeOldTier = escapeHtml(params.oldTier.toUpperCase());
  const safeNewTier = escapeHtml(params.newTier.toUpperCase());
  ...
  return sendEmail({
    ...
    html: `
      ...
      <p>${safeBusinessName} was downgraded from <strong>${safeOldTier}</strong> to <strong>${safeNewTier}</strong>.</p>
...
export async function sendOrderConfirmationEmail(
  ...
  const safeTitle = escapeHtml(orderDetails.productTitle);
  const safeOrderId = escapeHtml(orderDetails.orderId);
  const safeSubjectTitle = stripNewlines(orderDetails.productTitle);

  return sendEmail({
    to: email,
    subject: `Order Confirmation - ${safeSubjectTitle}`,
    html: `
...
export async function sendNewOrderNotificationEmail(
  ...
  return sendEmail({
    to: email,
    subject: `New Order - ${safeSubjectTitle}`,
    html: `
...
export async function sendRefundRequestEmail(
  ...
  const safeTitle = escapeHtml(refundDetails.productTitle);
  const safeOrderId = escapeHtml(refundDetails.orderId);
  const safeCustomerEmail = escapeHtml(refundDetails.customerEmail);
  const safeReason = escapeHtml(refundDetails.reason);
  const safeSubjectTitle = stripNewlines(refundDetails.productTitle);

  return sendEmail({
    to: email,
    subject: `Refund Request - ${safeSubjectTitle}`,
    html: `
...
export async function sendRefundConfirmationEmail(
  ...
  return sendEmail({
    to: email,
    subject: `Refund Processed - ${safeSubjectTitle}`,
    html: `
...
export async function sendDisputeNotificationEmail(
  ...
  const safeOrderId = escapeHtml(disputeDetails.orderId);
...
export async function sendFeaturedSlotExpiryEmail(
  ...
  const safeSuburbSubject = stripNewlines(suburb);
  ...
  return sendEmail({
    to: email,
    subject: `Featured Slot Expiring Soon - ${safeSuburbSubject}`,
    html: `
