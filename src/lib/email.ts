...
export async function sendTierDowngradeEmail(params: {
  ...
  return sendEmail({
    to: params.to,
    subject: stripNewlines(`Tier changed to ${params.newTier.toUpperCase()} — ${params.unpublishedCount} product(s) unpublished`),
    html: `
      <h1>Heads up from ${PLATFORM.NAME}</h1>
      <p>${escapeHtml(params.businessName)} was downgraded from <strong>${escapeHtml(params.oldTier.toUpperCase())}</strong> to <strong>${escapeHtml(params.newTier.toUpperCase())}</strong>.</p>
...
export async function sendOrderConfirmationEmail(
  ...
  return sendEmail({
    to: email,
    subject: stripNewlines(`Order Confirmation - ${orderDetails.productTitle}`),
    html: `
      <h1>Thank you for your purchase! 🎉</h1>
      <p>Your order has been confirmed.</p>
      <h2>Order Details</h2>
      <p><strong>Order ID:</strong> ${escapeHtml(orderDetails.orderId)}</p>
      <p><strong>Product:</strong> ${escapeHtml(orderDetails.productTitle)}</p>
...
export async function sendNewOrderNotificationEmail(
  ...
  return sendEmail({
    to: email,
    subject: stripNewlines(`New Order - ${orderDetails.productTitle}`),
    html: `
      <h1>You have a new order! 🎉</h1>
      <h2>Order Details</h2>
      <p><strong>Order ID:</strong> ${escapeHtml(orderDetails.orderId)}</p>
      <p><strong>Product:</strong> ${escapeHtml(orderDetails.productTitle)}</p>
      <p><strong>Customer:</strong> ${escapeHtml(orderDetails.customerEmail)}</p>
...
export async function sendRefundRequestEmail(
  ...
  return sendEmail({
    to: email,
    subject: stripNewlines(`Refund Request - ${refundDetails.productTitle}`),
    html: `
      <h1>Refund Request Received</h1>
      <p>A customer has requested a refund for one of your products.</p>
      <h2>Refund Details</h2>
      <p><strong>Order ID:</strong> ${escapeHtml(refundDetails.orderId)}</p>
      <p><strong>Product:</strong> ${escapeHtml(refundDetails.productTitle)}</p>
      <p><strong>Customer:</strong> ${escapeHtml(refundDetails.customerEmail)}</p>
      <p><strong>Refund Amount:</strong> A$${(refundDetails.amount / 100).toFixed(2)}</p>
      <p><strong>Reason:</strong> ${escapeHtml(refundDetails.reason)}</p>
...
export async function sendRefundConfirmationEmail(
  ...
  return sendEmail({
    to: email,
    subject: stripNewlines(`Refund Processed - ${refundDetails.productTitle}`),
    html: `
...
export async function sendDisputeNotificationEmail(
  ...
  return sendEmail({
    ...
    html: `
      ...
      <p><strong>Order ID:</strong> ${escapeHtml(disputeDetails.orderId)}</p>
      <p><strong>Amount:</strong> A$${(disputeDetails.amount / 100).toFixed(2)}</p>
      <p><strong>Reason:</strong> ${escapeHtml(disputeDetails.reason)}</p>
...
export async function sendFeaturedSlotExpiryEmail(
  ...
  return sendEmail({
    to: email,
    subject: stripNewlines(`Featured Slot Expiring Soon - ${suburb}`),
    html: `
      <h1>Your Featured Slot is Expiring Soon</h1>
      <p>Hi ${escapeHtml(businessName)},</p>
      <p>Your featured slot for <strong>${escapeHtml(suburb)}</strong> will expire on <strong>${formattedDate}</strong> (in ${daysRemaining} days).</p>
