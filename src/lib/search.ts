...
async function resolveFeaturedProfileMetadata(
  ...
  if (error) {
    ...
    return new Map<string, { suburbLabel: string | null; matchesSelection: boolean; regionId: number | null }>();
  }

  const normalizedSuburb = suburbTerm?.trim().toLowerCase() ?? null;

  const featureMap = new Map<string, { suburbLabel: string | null; matchesSelection: boolean; regionId: number | null }>();
  (data ?? []).forEach((slot) => {
