...
async function resolveFeaturedProfileMetadata(
  ...
  if (error) {
    ...
    return new Map<string, { suburbLabel: string | null; matchesSelection: boolean; regionId: number | string | null }>();
  }

  const normalizedSuburb = suburbTerm?.trim().toLowerCase() ?? null;

  const featureMap = new Map<string, { suburbLabel: string | null; matchesSelection: boolean; regionId: number | string | null }>();
  (data ?? []).forEach((slot) => {
