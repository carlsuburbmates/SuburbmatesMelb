  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "var(--bg-surface-1)", border: "1px solid var(--border)" }} data-testid="image-gallery">
      ...
      <div className="relative aspect-video group overflow-hidden" style={{ background: "var(--bg-surface-2)" }}>
        ...
        <button onClick={() => openModal(currentIndex)} className="absolute top-4 right-4 p-2.5 rounded-xl transition-all" style={{ background: "rgba(9,9,15,0.8)", backdropFilter: "blur(12px)", border: "1px solid var(--border)", color: "var(--text-primary)" }} aria-label="Zoom in image">
          <ZoomIn className="w-4 h-4" aria-hidden="true" />
        </button>
        {images.length > 1 && (
          <>
            <button onClick={prevImage} className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-xl transition-all" style={{ background: "rgba(9,9,15,0.8)", backdropFilter: "blur(12px)", border: "1px solid var(--border)", color: "var(--text-primary)" }} aria-label="Previous image"><ChevronLeft className="w-4 h-4" aria-hidden="true" /></button>
            <button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-xl transition-all" style={{ background: "rgba(9,9,15,0.8)", backdropFilter: "blur(12px)", border: "1px solid var(--border)", color: "var(--text-primary)" }} aria-label="Next image"><ChevronRight className="w-4 h-4" aria-hidden="true" /></button>
          </>
        )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ background: "rgba(9,9,15,0.95)" }} role="dialog" aria-modal="true">
          <div className="relative w-full h-full flex flex-col items-center justify-center p-4 md:p-12">
            ...
            <button onClick={closeModal} className="absolute top-6 right-6 p-3 rounded-xl transition-all" style={{ background: "rgba(255,255,255,0.06)", color: "var(--text-primary)" }} aria-label="Close image gallery"><X className="w-6 h-6" aria-hidden="true" /></button>
            {images.length > 1 && (
              <>
                <button onClick={prevImage} className="absolute left-6 top-1/2 -translate-y-1/2 p-3 rounded-xl transition-all" style={{ color: "var(--text-secondary)" }} aria-label="Previous image"><ChevronLeft className="w-8 h-8" aria-hidden="true" /></button>
                <button onClick={nextImage} className="absolute right-6 top-1/2 -translate-y-1/2 p-3 rounded-xl transition-all" style={{ color: "var(--text-secondary)" }} aria-label="Next image"><ChevronRight className="w-8 h-8" aria-hidden="true" /></button>
              </>
            )}
