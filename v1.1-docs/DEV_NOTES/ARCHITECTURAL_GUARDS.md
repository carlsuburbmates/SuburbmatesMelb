...
- MySQL (any usage)
- Legacy stack code from previous projects

---

## 5. External Code Constraints

External code is **reference only**, not implementation target.

**Forbidden to import directly:**

- Database access logic (queries, migrations, schema) from incompatible systems
- Framework-specific procedures (tRPC, etc.)
- Auth context and session handling from other systems
- Refund and dispute logic that conflicts with V1.1 model
- Money-flow logic assuming platform-as-MoR
- Any code encoding incompatible legal/compliance posture
...
- Artificial launch dates
- Any hard-coded cutover date

**Forbidden:**

- Hard-coding launch deadlines as behavior triggers
- Calendar-based behavior changes referencing external projects
