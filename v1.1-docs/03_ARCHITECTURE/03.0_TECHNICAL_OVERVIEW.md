...
-- Premium tier may eventually bundle a quota of featured slots, but MVP keeps this as a paid add-on for every tier.
```
...
> Search UX rule: users search/browse by suburb, category, or vendor name. We never surface "LGA" in copy, but every query maps to exactly one LGA behind the scenes so quota and featured logic stay deterministic.
