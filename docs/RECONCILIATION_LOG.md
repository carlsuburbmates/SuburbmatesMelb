...
| `origin/sentinel-html-sanitization-7338981564240011279` | Must Include | It is a parallel security branch with unique contact/email hardening not present on `origin/main`, and archived local routes depend on `@/lib/email` which that branch changes | Integrate after the `origin/main` merge is stabilized, then validate contact/email behavior and CI together |
