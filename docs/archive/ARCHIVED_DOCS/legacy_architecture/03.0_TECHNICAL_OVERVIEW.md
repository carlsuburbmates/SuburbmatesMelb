...
- **Subscription Billing:** Stripe Billing API (Directory/Basic/Pro listings + Featured Business add-on)
...
CREATE TABLE featured_slots (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  vendor_id UUID REFERENCES vendors(id), -- Directory, Basic, or Pro
  business_profile_id UUID REFERENCES business_profiles(id),
  lga_id INTEGER REFERENCES lgas(id),
  suburb_label VARCHAR(120), -- what the customer selected (UI)
  start_date TIMESTAMP NOT NULL,
  end_date TIMESTAMP NOT NULL, -- typically start_date + 30 days
  status ENUM('active', 'expired', 'cancelled') DEFAULT 'active',
  stripe_payment_intent_id VARCHAR(255),
  charged_amount_cents INTEGER (2000 for $20),
  created_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(lga_id, business_profile_id, start_date, end_date)
);
-- NOTE: Featured slots promote the vendor's public business profile (not an individual product).
-- Vendors of any tier can purchase the add-on; Stripe Checkout metadata persists vendor + business profile IDs and the target LGA/suburb.
-- Premium tier may eventually bundle a quota of featured slots, but MVP keeps this as a paid add-on.
```
...
CREATE TABLE featured_queue (
  ...
  created_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(vendor_id, lga_id) -- one position per vendor per LGA
);

-- Queue Position Algorithm (GAP #5 Solution):
-- position = ROW_NUMBER() OVER (PARTITION BY lga_id ORDER BY joined_at ASC)
-- estimated_wait_days = CEILING((position / featured_slot_cap) * 30)
```
...
```
> **UX Note:** Customers and business owners pick **suburbs** in the UI. Every suburb maps to a single LGA internally so ranking, quotas, and featured slots execute at the council level without exposing that term in the product.
...
> Search UX rule: users search/browse by suburb, category, or vendor name. We never surface "LGA" in copy, but every query maps to exactly one LGA behind the scenes so quota and featured logic stay deterministic. The resolver (`src/lib/suburb-resolver.ts`) maintains the suburb→council mapping from 08.0_MELBOURNE_SUBURBS_REFERENCE.md, and `/api/search` enforces tier-aware sorting (featured placements in the requested suburb → Premium → Standard → Basic → Directory).
